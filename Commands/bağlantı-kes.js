...
exports.commandSettings = {
    ...
    guildOnly: true, 
    coolDown: 3000, 
    description: "Bağlantı-Kes Luxer/ID"
}

  exports.run = async (client, message, args, embed, prefix) => { 
    if(!ayarlar.reg.regHammer.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('ADMINISTRATOR')) return; 
      ...
      if (!member) 
       ...
       message.lineReply("Bir üye belirtmelisin!").then(x=>x.delete({timeout:5000})) 
        ...
        .setColor("RANDOM")
        member.voice.kick()
        message.lineReply(sestensiktirettim).then(message => { message.delete({ timeout : 7500 }) }).then(m => message.react(ayarlar.emojiler.onay))  
  }