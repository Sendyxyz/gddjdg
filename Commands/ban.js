...
if(author.roles.cache.has(ayarlar.guildHammer.banHammer))    rolAl(author.id, ayarlar.guildHammer.banHammer) 
    embedCreator("DSASDA",`1 Saat içinde 3 tane ban attığın için ban yetkine el koydum!`,message,[true, 2000])
return
}
...
banEmbed("#DSA",`${emoji.luxer_banned} kişisi sunucumuz sınırlarından ${author} tarafından **__${reason}__** sebebi ile kovuldu`,message,[false])
...
if(banLimit.limit >= 7) {
    ...
    logEmbed(limitC,`
    ${author} kişisi  \`1 saatte\` \`3\` ban attığı için yetkilerini aldım Aşşağıda Son Attığı 7 yasaklama gözüküyor;  
...
exports.commandSettings = {
    ...
    guildOnly: true, 
    coolDown: 0, 
    description: "ban @Luxer/ID [Sebep]"
}