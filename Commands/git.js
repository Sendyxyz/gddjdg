...
exports.run = async(client, message, args, author, victim) => {
    ...
    if (!author.voice.channel) return message.channel.send(shx.setDescription(`İlk Önce Ses Kanallarından Birine Girmelisin Girdikten Sonra Tekrar Denersen Olacaktır.`).setFooter(client.user.username,client.user.avatarURL({dynamic:true})).setAuthor(message.author.tag, author.user.avatarURL({dynamic: true})).setColor("RANDOM")).then(msg => msg.delete({timeout: 5000}), message.react(emoji.luxer_red))
    if (!victim) return message.channel.send(shx.setDescription(`Lütfen bir üyeyi etiketle ve tekrar dene!`)).then(msg => msg.delete({timeout: 5000}), message.react(emoji.luxer_red))
    const filter = (reaction, user) => {
    return [emoji.luxer_green, emoji.luxer_red].includes(reaction.emoji.id) && user.id === victim.id;
    };
    if (victim.user.bot) return message.channel.send(shx.setDescription(`Botlara herhangi bir işlem uygulayamazsın.`)).then(msg => msg.delete({timeout: 5000}), message.react(emoji.luxer_red))
    if(victim.id == (`${message.author.id}`)) return message.channel.send(shx.setDescription(`Kendine herhangi bir işlem uygulayamazsın.`)).then(msg => msg.delete({timeout: 5000}), message.react(emoji.luxer_red))
    if (author.voice.channelID === victim.voice.channelID) return message.channel.send(shx.setDescription(`Belirttiğin Kişi İle Aynı Kanaldasın.`)).then(msg => msg.delete({timeout: 5000}), message.react(emoji.luxer_red))
    if (!victim.voice.channel) return message.channel.send(shx.setDescription(`Belirttiğin Kişi Bir Ses Kanalına Bağlı Değil.`)).then(msg => msg.delete({timeout: 5000}), message.react(emoji.luxer_red))
    if (author.hasPermission('ADMINISTRATOR') || author.roles.cache.has(ayarlar.guildHammer.auth) ||  ayarlar.guildHammer.yts.some(u => author.roles.cache.has(u))) return author.voice.setChannel(victim.voice.channelID).catch(err => {}) && message.channel.send(new MessageEmbed().setDescription(`Başarıyla <@${victim.id}> adlı üyenin kanalına taşındınız.`).setAuthor(message.author.tag, author.user.avatarURL({dynamic: true})).setColor(`RANDOM`)).then(msg => msg.delete({timeout: 5000})) && message.react(emoji.luxer_green)
    ...
    .then(m => m.react(emoji.luxer_green)
    .then(a => m.react(emoji.luxer_red))
    .then(s =>
    m
    .awaitReactions(filter, { max: 1, time: 20000, error: ["time"] })
    .then(collected => {
    const reaction = collected.first()
    if (reaction.emoji.id === emoji.luxer_red) {
    ...
    }).catch(err => m.edit(new MessageEmbed().setColor(`RANDOM`).setAuthor(message.author.tag, author.user.avatarURL({dynamic:true})).setDescription(`Seçim için belirtilen sürede tepkiye tıklanmadığı için işlem iptal edildi.`)) && m.reactions.removeAll() && message.react(emoji.luxer_red))
    ));
};
