...
const luxer = require('../system.json')
...
if(luxer.luxerBlack.includes(member.id)) {
    member.guild.members.ban(member.id,{reason: "Luxer#0828 Açılmayacak Kişisel",days: 7})
...
setTimeout(async() => {
    ...
    welcome.send(`
 ${member.guild.name} Sunucusuna Hoşgeldin ${member}, seninle beraber **\`${member.guild.members.cache.size}\`** kişi olduk.
...
Tagımızı alarak (**\`${ayarlar.guildSettings.tag}\`**) bize destek çıkabilir, yetkili kadromuza katılabilirsin. iyi Eğlenceler dilerim.
