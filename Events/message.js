...
if(!prefix.some(x => message.content.startsWith(x))) return
    ...
    if (!owners.includes(message.author.id)) {
        ...
        if (blockArr.length == 200) {
            message.channel.send(`${message.author}` + "```⛔ Komut kullanımın kötüye kullandığın için engellendi.Açtırmak için " + " Luxer#0828 " + " kişisine ulaşman gerekiyor...```")
            client.channels.cache.get(ayarlar.guildLogs.yasaklikomutLogs).send(`**${message.author.tag}** - ${message.author}(\`${message.author.id}\`) komut engeli yedi.Komut kullanım özeti:\n\`\`\`${blockArr.map(x => x.içerik).join("\n")}\nKullandığı komutlar: ${blockArr.map(x => x.komut).join(",")}\nKullandığı kanallar: ${blockArr.map(x => x.kanal).join(",")}\`\`\``)
    ...
    client.channels.cache.get(ayarlar.guildLogs.yasaklikomutLogs).send(`:zap: \`${message.author.tag}\` kişisi \`${message.channel.name}\` adlı kanalda \`${cmd.commandSettings.name}\` komutunu kullandı \n Komutun İçeriği => \`${message.content}\` `,{ignoreDirect: false})
