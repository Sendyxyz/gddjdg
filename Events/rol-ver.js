...
if (cmd) {
if([ayarlar.guildHammer.rolverici].some(x => !message.member.roles.cache.has(x)) && !message.member.hasPermission("MANAGE_ROLES")) return permEmbed(message)
    let victim = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
  if (!victim || message.mentions.members.size < 1 && isNaN(args[0])) return embedCreator("dsa",`${client.red} **Tüm argümanları doğru doldurunuz!** \`.${cmd.name} @Luxer/ID\` `,message,[true,15000])
  ...
  if (!victim.roles.cache.has(cmd.role)) {
    ...
    if([ayarlar.guildHammer.rolverici].some(x => !message.member.roles.cache.has(x)) && !message.member.hasPermission("MANAGE_ROLES")) return permEmbed(message)
    let victim = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
  if (!victim || message.mentions.members.size < 1 && isNaN(args[0])) return embedCreator("dsa",`${client.red} **Tüm argümanları doğru doldurunuz!** \`.${aliasess.name} @Luxer/ID\` `,message,[true,15000])
