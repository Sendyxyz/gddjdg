...
fs.readdir('./Commands', (err, files) => {
    ...
    files.forEach(f => {
        ...
        client.commands.set(props.commandSettings.name, props);
        props.commandSettings.aliases.forEach(alias => {
            client.aliases.set(alias, props.commandSettings.name);
        });
    });
})*/
...
//Event Handler
fs.readdir("./Events/", (err, Luxer) => {
    if (err) return console.error(err);
    Luxer = Luxer.filter(aru => aru.endsWith(".js"));
    if (Luxer.length == 0) return;
    console.log(`-----------------${Luxer.length} Eventlere Yüklemeye Geçiliyor ----------------`)
...
"Selaam bebek mugo ben kelebek",
"Dünyada ki en sinsi insansın!",
"Benim olmak ister misin??",
"Şerefsizsin.",
"Kedi olsan yine sana bakmam.",
"Benim yokluğumdа gülemez demişsin en çoktа onа güldüm!",
